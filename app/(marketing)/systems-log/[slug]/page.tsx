...
export async function generateStaticParams() {
  ...
  try {
    ...
    return entries
      .filter((e) => e.slug && isSystemsLogEntry(e))
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!entry || !isSystemsLogEntry(entry)) {
...
export default async function SystemsLogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  let entry;
  let blocks: NotionBlock[] = [];
  try {
    entry = await getBlogEntryBySlug(slug);
    if (!entry || !isSystemsLogEntry(entry)) notFound();
    blocks = await getPageBlocks(entry.id);
  } catch (e) {
    if (process.env.NODE_ENV === "development") console.error("[systems-log]", e);
    notFound();
  }
    ...
    <div className="min-h-screen relative">
      <div className="relative z-10 w-full px-4 sm:px-6 md:px-8 pt-14 pb-16">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <span className="text-[rgb(var(--neon)/0.4)]">&larr;</span> Back to Systems Log
          </Link>
